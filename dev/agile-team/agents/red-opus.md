...
Your pair partner is **any** green teammate — `green-haiku`, `green-sonnet`, or `green-opus`. Red and green tiers are assigned **independently** by the planner: your hard-to-design test suite may well be paired with a mechanical `green-haiku` implementation when the production code itself is simple once the tests pin down the behavior. Do **not** assume your partner is `green-opus` or shares your tier — check `TASKS.md` (column `Agent` on the `-green` row) or the `SPRINT.md` execution plan to learn which green variant is on this task. The **sprint-planner** arbitrates disputes between you and green.
