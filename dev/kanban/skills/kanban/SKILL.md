...
| `task.sh reject <task-path> [why]` | Reviewer refuted the close: bump the rejection counter, send the task back to `in_progress` for rework. Exits 3 and blocks the task past 2 rejections (stops the kanban). |
